...
public class TokoinService {
	...
	private List<String> processTokoinSearch(String key, String value, TokoinType tokoinType) throws Exception {
		...
		if (tokoinType.name().equals(TokoinType.USERS.name())) {
			UserField field = UserField.findBy(key);
			response = userService.userSearchProcess(tokoinType, key, value);
		}
		if (tokoinType.name().equals(TokoinType.TICKETS.name())) {
			TicketField field = TicketField.findBy(key);
			response = ticketsService.userSearchProcess(tokoinType, key, value);
		}
		if (tokoinType.name().equals(TokoinType.ORGANIZATION.name())) {
			OrganizationField field = OrganizationField.findBy(key);
			response = organizationService.userSearchProcess(tokoinType, key, value);
		}
		return response;
	}

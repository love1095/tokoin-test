...
public class TokoinUserService extends AbstractService implements UserService{
	...
	public List<DataResponse<User>> process(String term, String value) {
		...
		if (!users.isEmpty()) {
			for (User user : users) {
				...
				List<Ticket> assignees = findData(ticketArray, Ticket.class, TicketField.ASSIGNEE_ID.getValue(),
						userId);
				List<Ticket> submiters = findData(ticketArray, Ticket.class, TicketField.SUBMITTER_ID.getValue(),
						userId);
				List<Organization> organization = findData(organizationMapper.getEntityData(), Organization.class,
						OrganizationField.ID.getValue(), String.valueOf(user.getOrganizationId()));
				response.add(new DataResponse<User>(user, getResponse(assignees, submiters, organization)));
			}
		}
		return response;
	}
	...
	public Map<String, String> getResponse(List<Ticket> assignees, List<Ticket> submiters,
		...
		List<String> subjects = ListUtils.union(assignees, submiters).stream().map(Ticket::getSubject)
				.collect(Collectors.toList());
		LinkedHashMap<String, String> submitedSubjects = addItemInListToMap(subjects, TicketField.SUBJECT.getValue());
		List<String> organizationName = organization.stream().map(Organization::getName).collect(Collectors.toList());
		LinkedHashMap<String, String> organizationNames = addItemInListToMap(organizationName,

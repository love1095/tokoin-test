...
public class TokoinUserServiceTest {
    ...
    public void testUserSearchProcessShouldReturnCorrectDataWhenUsersAreFound() {
    	...
    	List<Ticket> ticket1s = List.of(ticket1);
    	List<Ticket> ticket2s = List.of(ticket2);
        ...
        doReturn(users).when(underTest).findData(userArray, User.class, SEARCH_TERM, SEARCH_VALUE);
        ...
        doReturn(ticket1s).when(underTest).findData(ticketArray, Ticket.class, ASSIGNEE_ID_TERM, idString);
		doReturn(ticket2s).when(underTest).findData(ticketArray, Ticket.class, SUBMITTER_ID_TERM, idString);
		doReturn(organizations).when(underTest).findData(organizationArray, Organization.class, ORGANIZATION_ID_TERM,
				organizationId);
		doReturn(relatedEntities).when(underTest).getResponse(ticket1s, ticket2s, organizations);
		...
		verify(underTest).findData(ticketArray, Ticket.class, SUBMITTER_ID_TERM, idString);
	...
	public void testUserSearchProcessShouldReturnEmptyDataWhenUsersAreNotFound() {
		...
		verify(underTest, never()).getResponse(anyListOf(Ticket.class), anyListOf(Ticket.class),
	...
	public void testGetResponseShouldReturnCorrectDataWhenCalled() {
    	...
    	Ticket ticket2 = mock(Ticket.class);
    	Organization organization = mock(Organization.class);
    	List<String> subjects = List.of(SUBJECT_1, SUBJECT_2);
		...
		// when
		Map<String, String> actual = underTest.getResponse(ticket1s, ticket2s, organizations);

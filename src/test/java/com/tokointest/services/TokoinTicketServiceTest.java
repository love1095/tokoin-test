...
public class TokoinTicketServiceTest {
    ...
    public void testProcessShouldReturnCorrectDataWhenTicketsAreFound() {
        ...
        doReturn(users1).when(underTest).findData(userArray, User.class, ID_TERM,
                String.valueOf(ASSIGNEE_ID));
        doReturn(users2).when(underTest).findData(userArray, User.class, ID_TERM,
                String.valueOf(SUBMITED_ID));
        doReturn(organizations).when(underTest).findData(organizationArray,
                Organization.class, ORGANIZATION_ID_TERM, organizationId);
        doReturn(relatedEntities).when(underTest).getResponse(users1, users2,
                organizations);
        ...
        verify(underTest).findData(userArray, User.class, ID_TERM,
                String.valueOf(ASSIGNEE_ID));
        verify(underTest).findData(userArray, User.class, ID_TERM,
                String.valueOf(SUBMITED_ID));
    ...
    public void testhProcessShouldReturnEmptyDataWhenTicketsAreNotFound() {
        ...
        verify(underTest, never()).getResponse(anyListOf(User.class),
                anyListOf(User.class), anyListOf(Organization.class));
    }
    ...
    public void testGetResponseShouldReturnCorrectDataWhenCalled() {
        ...
        User user2 = mock(User.class);
        Organization organization = mock(Organization.class);
        List<String> subjects = Arrays.asList(NAME_1, NAME_2);
        List<String> organizationNames = Arrays.asList(ORGANIZATION_NAME);
        List<User> user1s = Arrays.asList(user1);
        List<User> user2s = Arrays.asList(user2);
        List<Organization> organizations = Arrays.asList(organization);
